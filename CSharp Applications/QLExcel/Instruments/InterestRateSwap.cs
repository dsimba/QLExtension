    ...
    public class IRSwap
    {
            ...
            [ExcelArgument(Description = "trigger ")]object trigger)
        {
            ...
            try
            {
                ...
                if (genswaptpl.IsScheduleGiven)
                {
                    _GenericSwap.Type type = genswaptpl.SwapType == "Payer" ? _GenericSwap.Type.Payer : _GenericSwap.Type.Receiver;
                    
                    //************************** First Leg *******************************                    
                    DoubleVector notional1 = new DoubleVector();
                    foreach (var nl in  genswaptpl.FirstLegNotionals)
                    {
                        notional1.Add(nl);
                    }
                    DateVector sch1 = new DateVector();
                    foreach (var dt in genswaptpl.FirstLegSchedules)
                    {
                        sch1.Add(QLEX.QLConverter.DateTimeToDate(QLEX.QLConverter.StringToDateTime(dt)));
                    }

                    Calendar cal1 = QLEX.QLConverter.ConvertObject<Calendar>(genswaptpl.FirstLegCalendar);
                    DayCounter dc1 = QLEX.QLConverter.ConvertObject<DayCounter>(genswaptpl.FirstLegDayCounter);
                    BusinessDayConvention bdc1 = QLEX.QLConverter.ConvertObject<BusinessDayConvention>(genswaptpl.FirstLegConvention);
                    ...
                    foreach (var nl in  genswaptpl.SecondLegNotionals)
                    {
                        notional2.Add(nl);
                    }
                    DateVector sch2 = new DateVector();
                    foreach (var dt in genswaptpl.SecondLegSchedules)
                    {
                        sch2.Add(QLEX.QLConverter.DateTimeToDate(QLEX.QLConverter.StringToDateTime(dt)));
                    }

                    Calendar cal2 = QLEX.QLConverter.ConvertObject<Calendar>(genswaptpl.SecondLegCalendar);
                    DayCounter dc2 = QLEX.QLConverter.ConvertObject<DayCounter>(genswaptpl.SecondLegDayCounter);
                    BusinessDayConvention bdc2 = QLEX.QLConverter.ConvertObject<BusinessDayConvention>(genswaptpl.SecondLegConvention);
                    Schedule Schedule2 = new Schedule(sch2, cal2, bdc2);

                    IborIndex idx2;
                    idx2 = OHRepository.Instance.getObject<IborIndex>("IDX@"+genswaptpl.SecondLegIndex);
                    
                    //************************** swap ******************************
                    if (genswaptpl.FirstLegIndex == "FIXED")
                    {
                        swap = new GenericSwap(type, notional1, Schedule1, Schedule1, genswaptpl.FixedRate, dc1,
                            notional2, Schedule2, Schedule2, idx2, dc2, genswaptpl.Spread,
                            genswaptpl.SecondLegIndex.Contains("OIS") ? true : false);
                    }
                    else
                    {
                        idx1 = OHRepository.Instance.getObject<IborIndex>("IDX@" + genswaptpl.FirstLegIndex);
                        swap = new GenericSwap(type, notional1, Schedule1, Schedule1, idx1, dc1,
                            notional2, Schedule2, Schedule2, idx2, dc2, 0.0, genswaptpl.Spread,
                            genswaptpl.SecondLegIndex.Contains("OIS") ? true : false);
                    }
                ...
                else
                {
                    ...
                    DayCounter dc1 = QLEX.QLConverter.ConvertObject<DayCounter>(genswaptpl.FirstLegDayCounter);
                    BusinessDayConvention bdc1 = QLEX.QLConverter.ConvertObject<BusinessDayConvention>(genswaptpl.FirstLegConvention);
                    Date tradedate = QLEX.QLConverter.DateTimeToDate(QLEX.QLConverter.StringToDateTime(genswaptpl.TradeDate));
                    tradedate = cal1.adjust(tradedate);

                    Date startdate;
                    if (string.IsNullOrEmpty(genswaptpl.SettlementDate))
                    {
                        startdate = cal1.advance(tradedate, genswaptpl.FixingDays, TimeUnit.Days);
                    }
                    else
                    {
                        startdate = QLEX.QLConverter.DateTimeToDate(QLEX.QLConverter.StringToDateTime(genswaptpl.SettlementDate));
                    }

                    Date terminatedate;
                    if (string.IsNullOrEmpty(genswaptpl.MaturityDate))
                    {
                        // then tenor should not be zero
                        Period tenor = QLEX.QLConverter.ConvertObject<Period>(genswaptpl.Tenor);
                        Calendar nullcal = new NullCalendar();
                        terminatedate = nullcal.advance(startdate, tenor);
                    }
                    else
                    {
                        terminatedate = QLEX.QLConverter.DateTimeToDate(QLEX.QLConverter.StringToDateTime(genswaptpl.MaturityDate));
                    }

                    DateGeneration.Rule dgr1 = QLEX.QLConverter.ConvertObject<DateGeneration.Rule>(genswaptpl.FirstLegDateGenerationRule);

                    Frequency freq1 = QLEX.QLConverter.ConvertObject<Frequency>(genswaptpl.FirstLegFrequency);
                    Schedule schedule1 = new Schedule(startdate, terminatedate, new Period(freq1), cal1,
                        bdc1, bdc1, dgr1, genswaptpl.FirstLegEOM);
                    ...
                    DateGeneration.Rule dgr2 = QLEX.QLConverter.ConvertObject<DateGeneration.Rule>(genswaptpl.SecondLegDateGenerationRule);

                    Frequency freq2 = QLEX.QLConverter.ConvertObject<Frequency>(genswaptpl.SecondLegFrequency);
                    Schedule schedule2 = new Schedule(startdate, terminatedate, new Period(freq2), cal2,
                        bdc2, bdc2, dgr2, genswaptpl.SecondLegEOM);
