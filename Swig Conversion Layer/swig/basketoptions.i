...
typedef boost::shared_ptr<Payoff> SpreadBasketPayoffPtr;
%}
...
%rename(SpreadBasketPayoff) SpreadBasketPayoffPtr;
class SpreadBasketPayoffPtr : public BasketPayoffPtr  {
  public:
    %extend {
        SpreadBasketPayoffPtr(const boost::shared_ptr<Payoff> p) {
            return new SpreadBasketPayoffPtr(new SpreadBasketPayoff(p));
        }
    }
};
...
typedef boost::shared_ptr<PricingEngine> KirkEnginePtr;
%}

%rename(KirkEngine) KirkEnginePtr;
class KirkEnginePtr
    : public boost::shared_ptr<PricingEngine> {
  public:
    %extend {
        KirkEnginePtr(const BlackProcessPtr& process1,
            ...
            QL_REQUIRE(bsProcess2, "Black-Scholes process required");
            return new KirkEnginePtr(
                          new KirkEngine(bsProcess1,bsProcess2,correlation));
        }
    }
};
