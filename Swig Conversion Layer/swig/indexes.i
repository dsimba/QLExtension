...
%{
using QLExtension::CommodityIndexExt;
using QLExtension::CommodityCurveExt;
%}

%ignore CommodityIndexExt;
class CommodityIndexExt {
  public:
    std::string name() const;
	void update();
...
%template(CommodityIndexExt) boost::shared_ptr<CommodityIndexExt>;
%extend boost::shared_ptr<CommodityIndexExt> {
	boost::shared_ptr<CommodityCurveExt>(const std::string name,		// e.g. commod ng exchange
					   const boost::shared_ptr<CommodityCurveExt>& forwardCurve,
					   const Calendar& calendar)
	{
		return new boost::shared_ptr<CommodityIndexExt>(
		    new CommodityIndexExt(name, forwardCurve, 1, calendar));
	}
	boost::shared_ptr<CommodityCurveExt>(const std::string name,		// e.g. commod ng exchange
					   const Calendar& calendar)
	{
		return new boost::shared_ptr<CommodityIndexExt>(
		    new CommodityIndexExt(name, 1, calendar));
	}
}
IsObservable(boost::shared_ptr<CommodityIndexExt>);

%template(CommodityIndexExtHandle) Handle<CommodityIndexExt>;
IsObservable(Handle<CommodityIndexExt>);
%template(RelinkableCommodityIndexExtHandle)
RelinkableHandle<CommodityIndexExt>;
