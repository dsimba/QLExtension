...
// using QuantLib::BoostPhiloxUniformRng;
// using QuantLib::BoostThreefryUniformRng;
...
/*
class BoostThreefryUniformRng {
  public:
    BoostThreefryUniformRng(BigInteger seed = 0);
    Sample<Real> next() const;
	void discard(BigInteger n);
	void restart(BigInteger n);
};

class BoostPhiloxUniformRng {
  public:
    BoostPhiloxUniformRng(BigInteger seed = 0);
    Sample<Real> next() const;
	void discard(BigInteger n);
	void restart(BigInteger n);
};*/
...
template<class RNG, class F> class InverseCumulativeRng {
  public:
	...
	//InverseCumulativeRng(const RNG& rng, const F& ic);
    Sample<Real> next() const;
};
...
/*
// PRNG Gaussian
%template(InvCumulativePhiloxGaussianRng)
    InverseCumulativeRng<BoostPhiloxUniformRng,InverseCumulativeNormal>;
%template(InvCumulativeThreefryGaussianRng)
    InverseCumulativeRng<BoostThreefryUniformRng,InverseCumulativeNormal>;

// PRNG Student t
%template(InvCumulativePhiloxStudentRng)
    InverseCumulativeRng<BoostPhiloxUniformRng,InverseCumulativeStudent>;
%template(InvCumulativeThreefryStudentRng)
    InverseCumulativeRng<BoostThreefryUniformRng,InverseCumulativeStudent>;

// PRNG Poisson
%template(InvCumulativePhiloxPoissonRng)
    InverseCumulativeRng<BoostPhiloxUniformRng,InverseCumulativePoisson>;
%template(InvCumulativeThreefryPoissonRng)
    InverseCumulativeRng<BoostThreefryUniformRng,InverseCumulativePoisson>;
*/
...
%apply double INOUT[] { double* buffer1 }
%apply double INOUT[] { double* buffer2 }
%apply double INOUT[] { double* buffer3 }

%inline %{
/*void originalthreefry(long seed, long skip, long buffer[], long length) {
    return QLExtension::originalthreefry(seed, skip, buffer, length);
}*/

void uniformthreefry(long seed, long skip, double* buffer1, long length) {
	return QLExtension::uniformthreefry(seed, skip, buffer1, length);
}

void normalthreefry(long seed, long skip, double* buffer2, long length) {
    return QLExtension::normalthreefry(seed, skip, buffer2, length);
}

void gammathreefry(long seed, long skip, double* buffer3, long length, double shape, double scale) {
    return QLExtension::gammathreefry(seed, skip, buffer3, length, shape, scale);
}

%}

%clear double* buffer1;
%clear double* buffer2;
%clear double* buffer3;
