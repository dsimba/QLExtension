...
using QLExtension::IBOROISBasisRateHelper;
using QLExtension::FixedOISBasisRateHelper;
using QLExtension::IBORBasisRateHelper;
...
typedef boost::shared_ptr<RateHelper> IBOROISBasisRateHelperPtr;
typedef boost::shared_ptr<RateHelper> FixedOISBasisRateHelperPtr;
typedef boost::shared_ptr<RateHelper> IBORBasisRateHelperPtr;
%}
...
class FuturesRateHelperPtr : public boost::shared_ptr<RateHelper> {
  public:
    %extend {
		...
		// It seems BBG ends at IMM date
		FuturesRateHelperPtr(
                const Handle<Quote>& price, const Date& immDate, const Date& endDate,
				const DayCounter& dayCounter,
                const Handle<Quote>& convexityAdjustment) {
            return new FuturesRateHelperPtr(
                new FuturesRateHelper(price,immDate, endDate,
                                      dayCounter,convexityAdjustment));
        }
        FuturesRateHelperPtr(
                const Handle<Quote>& price,
...
%rename(IBOROISBasisRateHelper) IBOROISBasisRateHelperPtr;
class IBOROISBasisRateHelperPtr : public boost::shared_ptr<RateHelper> {
  public:
    %extend {
        IBOROISBasisRateHelperPtr(
				int settlementDays,
				const Period& tenor,
            ...
            return new IBOROISBasisRateHelperPtr(
                new IBOROISBasisRateHelper(settlementDays, tenor, overnightSpread, libor, ois, discountingCurve));
        }
        SwapPtr swap() {
            return boost::dynamic_pointer_cast<IBOROISBasisRateHelper>(*self)->swap();
        }
    }
};

%rename(FixedOISBasisRateHelper) FixedOISBasisRateHelperPtr;
class FixedOISBasisRateHelperPtr : public boost::shared_ptr<RateHelper> {
  public:
    %extend {
        FixedOISBasisRateHelperPtr(
				int settlementDays,
				const Period& tenor,
                const Handle<Quote>& overnightSpread,
            ...
            return new FixedOISBasisRateHelperPtr(
                new FixedOISBasisRateHelper(settlementDays, tenor, overnightSpread, fixedRate, fixedFrequency, fixedConvention, fixedDayCount,
					ois, overnightFrequency, discountingCurve));
        }
        SwapPtr swap() {
            return boost::dynamic_pointer_cast<FixedOISBasisRateHelper>(*self)->swap();
        }
    }
};

%rename(IBORBasisRateHelper) IBORBasisRateHelperPtr;
class IBORBasisRateHelperPtr : public boost::shared_ptr<RateHelper> {
  public:
    %extend {
        IBORBasisRateHelperPtr(
				int settlementDays,
				const Period& tenor,
                const Handle<Quote>& basis,
				const IborIndexPtr& baseLegIborIndex,
				const IborIndexPtr& basisLegIborIndex,
                const Handle<YieldTermStructure>& discountingCurve
							= Handle<YieldTermStructure>()) {
			boost::shared_ptr<IborIndex> libor1 =
                boost::dynamic_pointer_cast<IborIndex>(baseLegIborIndex);
			boost::shared_ptr<IborIndex> libor2 =
                boost::dynamic_pointer_cast<IborIndex>(basisLegIborIndex);
            return new IBORBasisRateHelperPtr(
                new IBORBasisRateHelper(settlementDays, tenor, basis, libor1, libor2, discountingCurve));
        }
        SwapPtr swap() {
            return boost::dynamic_pointer_cast<IBORBasisRateHelper>(*self)->swap();
        }
    }
};
